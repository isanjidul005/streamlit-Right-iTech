...
with tabs[0]:
    ...
    # Avg Present & Avg Absent capsules
    ...
    # Subject averages bar chart (instead of threshold split)
    ...
    if not marks_df.empty and "Marks" in marks_df.columns and "Subject" in marks_df.columns:
        ...
        if not subj_avg.empty:
            fig_subj = px.bar(subj_avg, x="Subject", y="Marks", color="Subject", color_discrete_map=SUBJECT_COLORS, title="Average marks by subject (filters applied)")
