...
with tabs[0]:
    ...
    # Threshold-filterable bar chart for student averages (bring back the good chart)
    ...
    if not marks_df.empty and "Marks" in marks_df.columns and "Name" in marks_df.columns:
        # compute per-student average across selected exams/subjects
        dfm = marks_df.copy()
        if subject_filter:
        ...
        if dfm.empty:
            ...
            # slider for threshold (within min/max)
            ...
            with st.expander("Explanation", expanded=auto_expand):
                st.write("This chart shows students grouped by whether their average marks (respecting filters) meet the chosen threshold. Use it to quickly find who meets expectations.")
    else:
